...
public final class HunterCommand implements CommandExecutor, TabCompleter {
    ...
    public HunterCommand(RoleService roleService, String adminPermission) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length != 2 || (!args[0].equalsIgnoreCase("add") && !args[0].equalsIgnoreCase("remove"))) {
            sender.sendMessage(Messages.info("Usage: /hunter add|remove <player>"));
            return true;
        }
        ...
        if (!roleService.removeHunter(target.getUniqueId())) {
            sender.sendMessage(Messages.error("Player is not a hunter."));
            return true;
        }
        Bukkit.broadcastMessage(Messages.ok(target.getName() + " is no longer a hunter!"));
        return true;
    }
